...
def main():
	...
	while count < frameCount:
		...
		if ret == True:
			...
			# check for blue
			rectangles_blue = template_matching(template_blue, frame_cloned, threshold)
			print('here')
			labels_blue = [1 for _ in rectangles_blue]
			labels += labels_blue
			bounding_box.append(rectangles_blue[0])

			# check for orange
			frame_cloned = np.copy(frame)
			rectangles_orange = template_matching(template_orange, frame_cloned, threshold)
